...
const ProductoEditScreen = ({ route, navigation }) => {
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              // ✅ Parsear el ID del producto
              const partes = (producto?.id || '').split('::');
              const categoriaId = partes[0] || categoria;
              const subcategoriaId = partes[1] || subcategoria;
              const productoId = producto?.id || 'nuevo';
  ...
  const guardarProducto = async () => {
    ...
    try {
      ...
      if (esNuevo) {
        // ✅ Al CREAR, enviar nombre, categoria y subcategoria
        ...
        // ✅ Al EDITAR: Siempre enviar "nombre" (el backend lo maneja automáticamente)
        datos.nombre = nombre.trim();
        ...
        console.log('📤 Actualizando producto:', producto.id);
