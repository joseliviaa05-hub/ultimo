...
const PedidosScreen = ({ navigation }) => {
  ...
  const [filtro, setFiltro] = useState('todos');
   ...
   useEffect(() => {
     const unsubscribe = navigation.addListener('focus', () => {
       // Recargar pedidos cada vez que la pantalla recibe focus
       cargarPedidos();
     });
  ...
  const cargarPedidos = async () => {
    try {
      ...
      // ✅ FIX: Extraer array de pedidos correctamente
      let pedidosArray = [];
      ...
      if (data.pedidos && Array.isArray(data.pedidos)) {
        ...
        pedidosArray = data;
      } else if (data.success && data.pedidos && Array.isArray(data.pedidos)) {
        pedidosArray = data.pedidos;
      } else {
      ...
      console.log(`✅ ${pedidosArray.length} pedidos procesados`);
      
      // ✅ Validar que sea un array antes de ordenar
      if (Array.isArray(pedidosArray)) {
        ...
        console.log('📦 Pedidos cargados y ordenados:', pedidosOrdenados.length);
      } else {
        console.error('❌ pedidosArray no es un array:', pedidosArray);
        setPedidos([]);
      }
