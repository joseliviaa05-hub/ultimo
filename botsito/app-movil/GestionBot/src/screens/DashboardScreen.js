...
const DashboardScreen = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      // ✅ FIX: Extraer estadísticas correctamente
      let stats;
      if (data.estadisticas && typeof data.estadisticas === 'object') {
        stats = data.estadisticas;
      } else if (data.success && data.estadisticas) {
        stats = data.estadisticas;
      } else {
        stats = data;
      }
      ...
      {estadisticas && (
        <>
          <View style={styles.statsGrid}>
            ...
            <StatsCard
              title="Total Vendido"
              value={`$${(estadisticas.total_vendido || estadisticas.total_ventas || 0).toLocaleString('es-AR')}`}
              icon="💰"
          ...
          {estadisticas.pedidos_por_dia && Array.isArray(estadisticas.pedidos_por_dia) && estadisticas.pedidos_por_dia.length > 0 && (
            <View style={styles.section}>
