...
class WhatsAppController {
    ...
    async sendMessage(req, res) {
        try {
            ...
            logger_1.logger.info(`📤 Mensaje de WhatsApp (MOCK) → ${to}: ${message.substring(0, 50)}`);
            ...
            logger_1.logger.error(`Error enviando mensaje de WhatsApp: ${error.message}`);
            res.status(500).json({
                success: false,
    ...
    async sendImage(req, res) {
        try {
            ...
            logger_1.logger.info(`📤 Imagen de WhatsApp (MOCK) → ${to}: ${imageUrl}`);
            ...
            logger_1.logger.error(`Error enviando imagen de WhatsApp: ${error.message}`);
            res.status(500).json({
                success: false,
